...
운영/개발 PC에서 일괄 확인(Ansible):

```bash
cd /mnt/c/Workspace/k8s-lab-dabin/vm-ansible
ansible app_vms -i inventory.ini -m shell -a "mount | grep /mnt/shared || true"
ansible app_vms -i inventory.ini -m shell -a "df -hT /mnt/shared"
ansible app_vms -i inventory.ini -m shell -a "grep /mnt/shared /etc/fstab"
```
...
```bash
bash /mnt/c/Workspace/k8s-lab-dabin/vm-ansible/nfs/nfs-server-diagnostics.sh
```
