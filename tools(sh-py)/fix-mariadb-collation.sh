...
# 5번째 인자: root 패스워드 (Ansible 호출 시 사용). 없으면 sudo mariadb fallback
ROOT_PASSWORD="${5:-}"

# root 접속 명령 결정: 패스워드 제공 시 TCP, 없으면 sudo 소켓
if [[ -n "$ROOT_PASSWORD" ]]; then
  MARIADB_ROOT="mariadb -h 127.0.0.1 -u root -p${ROOT_PASSWORD}"
else
  MARIADB_ROOT="sudo mariadb"
fi
...
$MARIADB_ROOT -e "SELECT VERSION();" || true
...
$MARIADB_ROOT -e "DROP DATABASE IF EXISTS \`${DB_NAME}\`;"
$MARIADB_ROOT -e "CREATE DATABASE \`${DB_NAME}\` CHARACTER SET utf8mb4 COLLATE ${TARGET_UTF8MB4_COLLATION};"

echo
echo "[6/6] Importing SQL"
$MARIADB_ROOT "$DB_NAME" < "$SQL_FILE"
